...
async function loadSoundConfig(configPath) {
  ...
  return {
    // Sound settings
    playOnSuccess:
      process.env.CLAUDE_HOOKS_SOUND_SUCCESS !== void 0
    ...
    cooldown:
      ...
      (fileConfig.settings?.cooldown ?? 2e3),
    // Filters
    minExecutionTime:
      parseInteger(process.env.CLAUDE_HOOKS_MIN_EXEC_TIME) ||
      (fileConfig.filters?.minExecutionTime ?? 1e3),
    // Debug
...
function playSound(soundFile, config, log) {
  try {
    ...
    if (config.delay > 0) {
      ...
      setTimeout(() => {
        ;(0, import_child_process.execSync)(`afplay "${soundFile}"`, {
          stdio: 'ignore',
        })
      }, config.delay)
    } else {
