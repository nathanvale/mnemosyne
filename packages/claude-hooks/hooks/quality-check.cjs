...
function findProjectRoot(startPath) {
  ...
  while (currentPath !== '/') {
    try {
      const stat = (0, import_fs2.statSync)(
        import_path2.default.join(currentPath, 'package.json'),
      )
      if (stat.isFile()) {
        return currentPath
      }
...
function generateDummyContent(parsedImport) {
  ...
  lines.push(
    ' * This file was created to satisfy ESLint during test-driven development.',
  )
...
async function createDummyFile(filePath, parsedImport, log) {
  try {
    ...
    if (log) {
      log(
        `\u{1F4DD} Created dummy implementation: ${import_path3.default.relative(process.cwd(), filePath)}`,
      )
          ...
          if (importErrors.length > 0) {
            ...
            try {
              ...
              for (const importError of importErrors) {
                ...
                const fullPath = import_path5.default.join(
                  projectRoot,
                  resolvedPath + ext,
                )
...
async function createPrettierChecker(filePath, config, log) {
  ...
  let prettier
  try {
    const prettierPath = import_path6.default.join(
      projectRoot,
      'node_modules',
      'prettier',
      'index.cjs',
    ...
    log.debug(
      `Prettier not found in project - will skip Prettier checks. Error: ${error}`,
    )
    return null
  }
  return {
...
async function createTypeScriptChecker(filePath, config, log, tsConfigCache) {
  ...
  log.debug(
    `Looking for TypeScript at: ${import_path7.default.join(projectRoot, 'node_modules', 'typescript')}`,
  )
  let ts
  try {
    ts = await import(
  ...
  return {
    async check() {
      ...
      try {
        ...
        log.debug(
          `Using TypeScript config: ${import_path7.default.basename(configPath)} for ${import_path7.default.relative(projectRoot, filePath)}`,
        )
        const configFile = ts.readConfigFile(configPath, ts.sys.readFile)
        const parsedConfig = ts.parseJsonConfigFileContent(
          configFile.config,
          ts.sys,
          import_path7.default.dirname(configPath),
        )
          ...
          if (importDiagnostics.length > 0) {
            ...
            try {
              ...
              for (const diagnostic of importDiagnostics) {
                ...
                const fullPath = import_path7.default.join(
                  projectRoot,
                  resolvedPath + ext,
                )
                if (!(await fileExists(fullPath))) {
        ...
        if (editedFileDiagnostics.length > 0) {
          errors.push(
            `TypeScript errors in edited file (using ${import_path7.default.basename(configPath)})`,
          )
...
var TypeScriptConfigCache = class {
  ...
  getTsConfigForFile(filePath) {
    ...
    const relativePath = import_path8.default.relative(
      this.projectRoot,
      filePath,
    )
    ...
    if (relativePath.includes('webview/')) {
      const webviewConfig = import_path8.default.join(
        this.projectRoot,
        'tsconfig.webview.json',
      )
    ...
    if (relativePath.includes('.test.') || relativePath.includes('.spec.')) {
      const testConfig = import_path8.default.join(
        this.projectRoot,
        'tsconfig.test.json',
      )
  ...
  async suggestRelatedTests(log) {
    ...
    for (const ext of testExtensions) {
      if (await fileExists(`${baseName}.${ext}`)) {
        hasTests = true
        log.warning(
          `\u{1F4A1} Related test found: ${import_path9.default.basename(baseName)}.${ext}`,
        )
    ...
    if (!hasTests) {
      ...
      for (const ext of testExtensions) {
        if (
          ...
          hasTests = true
          log.warning(
            `\u{1F4A1} Related test found: __tests__/${baseFileName}.${ext}`,
          )
          log.warning('   Consider running the tests to ensure nothing broke')
          break
        }
    ...
    if (!hasTests) {
      log.warning(
        `\u{1F4A1} No test file found for ${import_path9.default.basename(this.filePath)}`,
      )
...
${colors.red}Found ${errors.length} issue(s) that MUST be fixed!${colors.reset}`)
    ...
    console.error(
      `${colors.red}\u274C ALL ISSUES ARE BLOCKING \u274C${colors.reset}`,
    )
    console.error(
      `${colors.red}\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550${colors.reset}`,
    )
...
async function main() {
  ...
  console.error('')
  console.error(
    `\u269B\uFE0F  React App Quality Check v${hookVersion} - Starting...`,
  )
  console.error(
    '\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500',
  )
...
${colors.green}\u2705 No checks needed for ${import_path9.default.basename(filePath)}${colors.reset}`)
  ...
  console.error('')
  console.error(
    `\u{1F50D} Validating: ${import_path9.default.basename(filePath)}`,
  )
  console.error(
    '\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500',
  )
