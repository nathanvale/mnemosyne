...
export abstract class BaseHook<
  TEvent extends ClaudeHookEvent = ClaudeHookEvent,
> {
  ...
  protected async parseEvent(): Promise<TEvent | null> {
    try {
      ...
      if (event.hook_event_name === this.eventType) {
        ...
        this.log.debug(
          `Parsed ${this.eventType} event: ${JSON.stringify(event)}`,
        )
        return event
      } else {
        this.log.warning(
          `Expected ${this.eventType} event, got type=${event.type}, hook_event_name=${event.hook_event_name}`,
        )
        return null
      }
