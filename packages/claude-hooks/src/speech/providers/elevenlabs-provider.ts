...
export class ElevenLabsProvider extends BaseTTSProvider {
  ...
  async getVoices(): Promise<Voice[]> {
    ...
    try {
      ...
      const list: Array<VoiceApi> = Array.isArray(result?.voices)
        ? result.voices!
        : []
  ...
  configure(config: TTSProviderConfig): void {
    ...
    if (c.outputFormat !== undefined)
      this.elConfig.outputFormat = c.outputFormat
    if (c.stability !== undefined)
      this.elConfig.stability = this.clamp01(c.stability)
    ...
    if (c.enableLogging !== undefined)
      this.elConfig.enableLogging = c.enableLogging
  }
