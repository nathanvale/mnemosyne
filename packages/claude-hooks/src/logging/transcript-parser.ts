...
export class TranscriptParser {
  private readonly config: TranscriptParserConfig
  ...
  getSummary(transcript: ParsedTranscript): {
    ...
    const userTurns = transcript.turns.filter(
      (t) => t.role === 'user' || t.role === 'human',
    ).length
    const assistantTurns = transcript.turns.filter(
      (t) => t.role === 'assistant',
    ).length
