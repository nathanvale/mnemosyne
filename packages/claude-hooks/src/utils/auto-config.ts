  ...
  if (typeof obj === 'string') {
    ...
    return obj.replace(/\$\{([^}]+)\}/g, (match, varName) => {
      ...
      if (envValue === undefined) {
        console.warn(
          `Warning: Environment variable ${varName} not found, keeping placeholder`,
        )
...
function getConfigSearchPaths(hookName: string): string[] {
  ...
  // eslint-disable-next-line turbo/no-undeclared-env-vars
  if (process.env.CLAUDE_HOOKS_CONFIG_DIR) {
    // eslint-disable-next-line turbo/no-undeclared-env-vars
  ...
  if (monorepoRootFromOriginal) {
    paths.push(
      path.join(monorepoRootFromOriginal, '.claude', 'hooks', configFileName),
    )
  }
  ...
  paths.push(
    path.join(projectRootFromOriginal, '.claude', 'hooks', configFileName),
  )
  ...
  if (currentCwd !== originalCwd) {
    ...
    if (
      ...
      paths.push(
        path.join(monorepoRootFromCurrent, '.claude', 'hooks', configFileName),
      )
    }
    ...
    if (projectRootFromCurrent !== projectRootFromOriginal) {
      paths.push(
        path.join(projectRootFromCurrent, '.claude', 'hooks', configFileName),
      )
    }
  }
...
export async function loadAutoConfig<T extends Record<string, unknown>>(
  ...
  if (configFilePath && typeof configWithEnv === 'object') {
    ;(configWithEnv as unknown as Record<string, unknown>).__configPath =
      configFilePath
  }
...
export function getAutoConfigPath(hookName: string): string {
  ...
  return (
    searchPaths[0] ||
    path.join(process.cwd(), '.claude', 'hooks', `${hookName}.config.json`)
  )
}
