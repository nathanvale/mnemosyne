...
async function getMacOSVoices(): Promise<Voice[]> {
  try {
    ...
    logger.warning(`Failed to get macOS voices: ${error}`)
    return []
  }
}
...
async function getElevenLabsVoices(apiKey?: string): Promise<Voice[]> {
  try {
    ...
    logger.warning(`Failed to get ElevenLabs voices: ${error}`)
    return []
  }
}
  ...
  try {
    switch (provider) {
      case 'openai': {
        ...
        if (!result.success) {
          logger.error(`Preview failed: ${result.error}`)
        }
        break
      }
...
ENVIRONMENT VARIABLES:
  ...
  if (options.provider === 'all' || options.provider === 'elevenlabs') {
    ...
    logger.info(
      `Found ${voicesByProvider['ElevenLabs'].length} ElevenLabs voices`,
    )
  }
  ...
  const output =
    options.format === 'json'
      ? formatAsJSON(voicesByProvider)
      : formatAsTable(voicesByProvider)
