...
export class StopHook extends BaseHook<ClaudeStopEvent> {
  ...
  private async handleSpeech(event: ClaudeStopEvent): Promise<void> {
    ...
    try {
      const speechSuccess = await this.speechEngine.speak(speechMessage)
      if (speechSuccess) {
  ...
  private async processTranscript(transcriptPath: string): Promise<void> {
    try {
      ...
      const storedTranscript =
        await this.transcriptParser.parseAndStoreTranscript(transcriptPath)
      ...
      const lastMessage =
        this.transcriptParser.getLastAssistantMessage(transcript)
      if (lastMessage) {
