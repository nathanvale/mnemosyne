export class SelectiveEscalation {
  ...
  shouldEscalate(criteria: {
    ...
    const currentRate =
      criteria.totalChecks > 0
        ? criteria.previousEscalations / criteria.totalChecks
        : 0
  ...
  calculateComplexity(error: string): number {
    ...
    if (error.includes('Type instantiation is excessively deep'))
      complexity += 0.8
  ...
  private updateAdaptiveThreshold(): void {
    ...
    const recentEscalationRate =
      this.recentChecks.filter((c) => c.escalated).length /
      this.recentChecks.length
    const averageErrorCount =
      this.recentChecks.reduce((sum, c) => sum + c.errorCount, 0) /
      this.recentChecks.length
