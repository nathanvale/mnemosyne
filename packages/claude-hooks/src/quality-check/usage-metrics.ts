export class UsageMetrics {
  ...
  recordErrorPattern(data: {
    ...
    this.metrics.errorPatterns.set(
      pattern,
      (this.metrics.errorPatterns.get(pattern) ?? 0) + 1,
    )
  }
  ...
  getHourlyStatistics() {
    ...
    const recentData = this.metrics.hourlyData.filter(
  ...
  getDashboard() {
    ...
    const avgPerformance =
      this.performanceData.length > 0
        ? this.performanceData.reduce((sum, p) => sum + p.duration, 0) /
...
class CostTracker {
  ...
  getMonthlyEstimate() {
    const avgTokens =
      this.invocations.length > 0
        ? this.invocations.reduce(
            (sum, inv) => sum + inv.promptTokens + inv.completionTokens,
            0,
  ...
  getCostByCategory() {
    const categories: Record<
      string,
      { invocations: number; averageTokens: number }
    > = {}

    this.invocations.forEach((inv) => {
      if (inv.category) {
        if (!categories[inv.category]) {
          categories[inv.category] = { invocations: 0, averageTokens: 0 }
        }
        categories[inv.category].invocations++
        categories[inv.category].averageTokens +=
          inv.promptTokens + inv.completionTokens
      }
    })

    Object.keys(categories).forEach((cat) => {
      categories[cat].averageTokens =
        categories[cat].averageTokens / categories[cat].invocations
    })
  ...
  getCachingSavings() {
    const cachedInvocations = this.invocations.filter(
      (inv) => inv.cached,
    ).length
