export class PerformanceMonitor {
  ...
  getMemoryMetrics(operationName: string) {
    const tracking = this.memoryTracking.get(operationName)
    if (!tracking) {
  ...
  getCacheStatistics() {
    ...
    this.cacheStats.forEach((data, name) => {
  ...
  getRecommendations(): string[] {
    ...
    this.cacheStats.forEach((data) => {
      const hitRate =
        data.hits + data.misses > 0 ? data.hits / (data.hits + data.misses) : 0
      if (hitRate < 0.5) {
