...
export class SubAgentContextBuilder {
  ...
  async buildContext(
    errors: string[],
    filePath: string,
  ): Promise<EnrichedSubAgentContext> {
  ...
  private extractErrorDetails(errors: string[]): TypeScriptErrorDetail[] {
    return errors.map((error) => {
      ...
      const lineNumber = lineMatch
        ? parseInt(lineMatch[1] || lineMatch[2], 10)
        : undefined
  ...
  private analyzeProjectPatterns(filePath: string): EnrichedProjectContext {
    const isMonorepo =
      filePath.includes('/packages/') || filePath.includes('/apps/')
    const packagePattern = isMonorepo ? '@studio/*' : undefined
    ...
    if (isMonorepo && filePath.includes('/src/')) {
      ...
      componentPath =
        lastSlash > 0 ? `src/${afterSrc.substring(0, lastSlash)}` : undefined
    }
