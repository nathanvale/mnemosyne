...
export function loadConfigFromEnv(): Partial<SubAgentConfig> {
  ...
  if (process.env.CLAUDE_HOOKS_SUBAGENT_MAX_ESCALATION_RATE) {
    const rate = parseFloat(
      process.env.CLAUDE_HOOKS_SUBAGENT_MAX_ESCALATION_RATE,
    )
  ...
  if (process.env.CLAUDE_HOOKS_SUBAGENT_MONTHLY_COST_LIMIT) {
    const limit = parseFloat(
      process.env.CLAUDE_HOOKS_SUBAGENT_MONTHLY_COST_LIMIT,
    )
  ...
  if (process.env.CLAUDE_HOOKS_SUBAGENT_MAX_FAILURES) {
    const maxFailures = parseInt(
      process.env.CLAUDE_HOOKS_SUBAGENT_MAX_FAILURES,
      10,
    )
...
export function mergeConfig(
  ...configs: Partial<SubAgentConfig>[]
): SubAgentConfig {
...
export function validateConfig(config: SubAgentConfig): string[] {
  ...
  for (const pattern of [
    ...config.alwaysEscalatePatterns,
    ...config.neverEscalatePatterns,
  ]) {
    try {
      new RegExp(pattern)
    } catch {
    ...
    switch (arg) {
      ...
      case '--subagent-sensitivity':
        if (['conservative', 'balanced', 'aggressive'].includes(nextArg)) {
          config.escalationSensitivity = nextArg as
            | 'conservative'
            | 'balanced'
            | 'aggressive'
          i++
