...
export class SubAgentOrchestrator {
   ...
   */
  formatIntegratedOutput(
    errors: string[],
    analysis: SubAgentAnalysis | null,
  ): string {
    let output = ''
  ...
  estimateMonthlyCost(): number {
    ...
    const tokensPerMonth =
      (this.metrics.totalTokensUsed / this.metrics.totalInvocations) *
      30 *
      24 *
      2 // Assuming 2 invocations per hour
  ...
  private updateAverageResponseTime(responseTime: number): void {
    const totalTime =
      this.metrics.averageResponseTime *
        (this.metrics.successfulInvocations - 1) +
      responseTime
    this.metrics.averageResponseTime =
      totalTime / this.metrics.successfulInvocations
  }
}
