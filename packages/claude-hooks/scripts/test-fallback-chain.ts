...
async function testScenario1() {
  ...
  log(
    'Creating provider with auto mode (should select ElevenLabs)...',
    'yellow',
  )
...
async function testScenario4() {
  header(
    'TEST 4: Invalid keys for both ElevenLabs and OpenAI → Should use macOS',
  )
  ...
  const fullChain = new FallbackProvider(
    elevenlabsProvider,
    openaiWithMacOSFallback,
  )
...
async function main() {
  ...
  log(
    'This test validates the fallback chain: ElevenLabs → OpenAI → macOS\n',
    'blue',
  )
