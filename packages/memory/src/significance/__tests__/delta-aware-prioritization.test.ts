...
describe('Delta-Aware Memory Prioritization - Task 5.5', () => {
  ...
  describe('Emotional Transition-Based Ranking', () => {
    ...
    it('should handle memories with multiple delta types and prioritize by emotional impact', async () => {
      const volatileMemory = createMemoryWithComplexDeltas([
        ...
        { previous: 7.2, current: 5.8, type: 'stabilization', significance: 0.72 },
      ])
  ...
  describe('Delta Impact Weighting', () => {
    ...
    it('should integrate delta timeline context in prioritization', async () => {
      ...
      const historicalTime = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString() // 1 week ago
  ...
  describe('Edge Cases and Complex Scenarios', () => {
    it('should handle memories with no detectable mood deltas appropriately', async () => {
      ...
      const memories = [noDeltaMemory, subtleDeltaMemory, significantDeltaMemory]
      const prioritized = await prioritizer.prioritizeBySignificance(memories)

      expect(prioritized[0].id).toBe(significantDeltaMemory.id)
      // No-delta and subtle-delta priorities depend on other factors
      expect([noDeltaMemory.id, subtleDeltaMemory.id]).toContain(prioritized[1].id)
      expect([noDeltaMemory.id, subtleDeltaMemory.id]).toContain(prioritized[2].id)
    })
  ...
  function createMemoryWithMoodDelta(deltaConfig: {
    ...
    const moodDelta: MoodDelta = {
      ...
      direction: deltaConfig.current > deltaConfig.previous ? 'positive' : 'negative',
      confidence: deltaConfig.confidence || 0.80,
    ...
    const emotionalAnalysis: EmotionalAnalysis = {
      context: {
        primaryEmotion: deltaConfig.current > 6 ? EmotionalState.JOY : 
                       deltaConfig.current < 4 ? EmotionalState.SADNESS : 
      ...
      moodScoring: {
        ...
        confidence: deltaConfig.confidence || 0.80,
        factors: [],
        descriptors: deltaConfig.current > 6 ? ['positive', 'improving'] : 
                     deltaConfig.current < 4 ? ['negative', 'declining'] : ['stable', 'neutral'],
        delta: extendedMoodDelta as MoodDelta,
      },
      trajectory: {
        direction: deltaConfig.current > deltaConfig.previous ? 'improving' : 'declining',
        significance: deltaConfig.significance,
        points: [
          {
        ...
        turningPoints: deltaConfig.significance > 0.8 ? [
          {
            timestamp: new Date(),
            type: deltaConfig.transitionType === 'crisis' ? 'setback' : 'support_received',
            magnitude: Math.abs(deltaConfig.current - deltaConfig.previous),
      ...
      patterns: [
        {
          ...
          evidence: [`mood_change_from_${deltaConfig.previous}_to_${deltaConfig.current}`],
          significance: deltaConfig.significance,
        },
      ],
    }
  ...
  function createMemoryWithComplexDeltas(deltaConfigs: Array<{
    previous: number
    current: number
    type: string
    significance: number
  ...
  function createMemoryWithConflictDelta(config: {
    ...
    return createMemoryWithMoodDelta({
      ...
      transitionType: config.conflictType === 'escalation' ? 'crisis' : 'resolution',
      confidence: 0.89,
      urgencyIndicators: config.conflictType === 'escalation' ? ['conflict', 'escalation'] : undefined,
    })
  }
  ...
  function createMemoryWithMixedDeltas(config: {
    positiveAspects: Array<{ emotion: string; delta: number; confidence: number }>
    negativeAspects: Array<{ emotion: string; delta: number; confidence: number }>
    overallComplexity: number
  }): ExtractedMemory {
    // Calculate net mood change from mixed emotions
    const positiveSum = config.positiveAspects.reduce((sum, aspect) => sum + aspect.delta, 0)
    const negativeSum = config.negativeAspects.reduce((sum, aspect) => sum + aspect.delta, 0)
