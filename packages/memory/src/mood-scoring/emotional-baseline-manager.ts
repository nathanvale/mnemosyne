...
export interface EmotionalBaseline {
  ...
  relationshipPatterns: Record<string, {
    averageMood: number
    confidence: number
    dataPoints: number
  }>
...
export interface DeviationAnalysis {
  ...
  /** Type of deviation */
  deviationType: 'significant_decline' | 'significant_elevation' | 'normal_variation'
...
export class EmotionalBaselineManager {
  ...
  establishBaseline(
    userId: string, 
    historicalConversations: (ConversationData & { moodAnalysis: MoodAnalysisResult })[]
  ): EmotionalBaseline {
    ...
    // Extract mood scores
    const moodScores = historicalConversations.map(conv => conv.moodAnalysis.score)
    ...
    const baseConfidence = Math.min(0.95, 0.5 + (historicalConversations.length / 20))
    ...
    const temporalPatterns = this.analyzeTemporalPatterns(historicalConversations)
    
    // Analyze relationship patterns
    const relationshipPatterns = this.analyzeRelationshipPatterns(historicalConversations)
  ...
  analyzeDeviation(userId: string, conversation: ConversationData & { moodAnalysis: MoodAnalysisResult }): DeviationAnalysis {
    ...
    const zScore = (currentScore - comparisonBaseline) / Math.max(0.5, comparisonVolatility)
    ...
    if (deviationMagnitude >= 2.0) {
      deviationType = currentScore > comparisonBaseline ? 'significant_elevation' : 'significant_decline'
    } else {
    ...
    const contextualSignificance: 'high' | 'medium' | 'low' = 
      deviationMagnitude >= 2.0 ? 'high' : 
      deviationMagnitude >= 1.0 ? 'medium' : 'low'
    
    // Recommended actions
    const recommendedAction: string[] = []
    if (deviationType === 'significant_decline') {
      recommendedAction.push('monitor', 'check_for_triggers', 'consider_support')
    } else if (deviationType === 'significant_elevation') {
      recommendedAction.push('celebrate', 'identify_positive_factors', 'assess_sustainability')
    } else {
      recommendedAction.push('continue_monitoring')
    }
    
    // Potential triggers and sustainability assessment
    const potentialTriggers = deviationType !== 'normal_variation' ? 
      ['relationship_changes', 'life_events', 'environmental_factors'] : undefined
    
    const sustainabilityAssessment = deviationType === 'significant_elevation' ? {
      likely: deviationMagnitude < 3.0, // More moderate elevations are more sustainable
      factors: ['baseline_consistency', 'support_systems', 'coping_mechanisms']
    } : undefined
  ...
  shouldUpdateBaseline(
    userId: string, 
    newConversations: (ConversationData & { moodAnalysis: MoodAnalysisResult })[]
  ): boolean {
  ...
  updateBaseline(
    ...
    if (!currentBaseline) {
      throw new Error(`Cannot update baseline: no existing baseline found for user ${userId}`)
    }
    ...
    const updatedAverage = (currentBaseline.averageMood * existingWeight + newAverage * newWeight) / totalWeight
    ...
    const combinedVariance = ((currentBaseline.variationPattern.volatility ** 2) * existingWeight + 
      this.calculateVariance(newScores, newAverage) * newWeight) / totalWeight
    ...
    const updatedBaseline: EmotionalBaseline = {
      ...
      variationPattern: {
        ...
        cyclicalTendency: newVolatility > 2.0 ? 'high' : newVolatility > 1.0 ? 'medium' : 'low'
      },
  ...
  private analyzeTemporalPatterns(
    ...
    conversations.forEach(conv => {
      ...
      const dayName = conv.timestamp.toLocaleDateString('en-US', { weekday: 'long' }).toLowerCase()
  ...
  private analyzeRelationshipPatterns(
    ...
    const relationshipPatterns: Record<string, {
      averageMood: number
      confidence: number
      dataPoints: number
    }> = {}
  ...
  private calculateVariance(scores: number[], average: number): number {
    ...
    return scores.reduce((sum, score) => sum + Math.pow(score - average, 2), 0) / scores.length
  }
}