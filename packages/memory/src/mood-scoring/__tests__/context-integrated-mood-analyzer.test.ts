...
describe('Context-Integrated Mood Analysis - Relationship-Aware Scoring (Task 3.7)', () => {
  ...
  describe('Contextual Factor Integration', () => {
    it('should enhance mood analysis with vulnerability context for lower-scoring conversations', async () => {
      ...
      const contextualFactors = contextBuilder.identifyContextualFactors(vulnerableConversation)
      const relationshipDynamics = relationshipAnalyzer.analyzeRelationshipDynamics(vulnerableConversation)
      
      // Context should indicate high vulnerability and intimacy
      expect(contextualFactors.primaryTriggers).toContain('vulnerability_expression')
      ...
      expect(contextualFactors.contextualSummary!.recommendedMoodAdjustments).toContain('baseline_elevation')
      expect(contextualFactors.relationshipSpecificFactors.intimacyLevel).toBe('high')
      expect(contextualFactors.relationshipSpecificFactors.trustDependency).toBe('high')
    })
    ...
    it('should apply conflict context penalties for escalating relationship conflicts', async () => {
      ...
      // Context-enhanced analysis
      const contextualFactors = contextBuilder.identifyContextualFactors(conflictConversation)
      const relationshipDynamics = relationshipAnalyzer.analyzeRelationshipDynamics(conflictConversation)
      ...
      expect(contextualFactors.relationshipSpecificFactors.conflictHistory).toBe('escalating')
      expect(contextualFactors.relationshipSpecificFactors.communicationBreakdown).toBe(true)
      ...
      expect(contextualFactors.emotionalSafetyFactors).toContain('dismissal_patterns')
      expect(contextualFactors.emotionalSafetyFactors).toContain('defensive_positioning')
    })
    ...
    it('should boost mood scores for achievement contexts with strong support systems', async () => {
      ...
      // Context-enhanced analysis
      const contextualFactors = contextBuilder.identifyContextualFactors(celebrationConversation)
      const relationshipDynamics = relationshipAnalyzer.analyzeRelationshipDynamics(celebrationConversation)
      
      // Context should indicate achievement and strong support
      expect(contextualFactors.primaryTriggers).toContain('achievement_recognition')
      expect(contextualFactors.primaryTriggers).toContain('positive_reinforcement')
      expect(contextualFactors.relationshipSpecificFactors.supportLevel).toBe('high')
      expect(contextualFactors.relationshipSpecificFactors.validationPatterns).toContain('achievement_celebration')
      ...
      expect(contextualFactors.contextualSummary!.recommendedMoodAdjustments).toContain('baseline_elevation')
      expect(contextualFactors.emotionalSafetyFactors).toContain('unconditional_support')
    })
    ...
    it('should apply therapeutic context adjustments for professional counseling interactions', async () => {
      ...
      // Context-enhanced analysis
      const contextualFactors = contextBuilder.identifyContextualFactors(therapeuticConversation)
      const relationshipDynamics = relationshipAnalyzer.analyzeRelationshipDynamics(therapeuticConversation)
      ...
      expect(contextualFactors.therapeuticFactors!.breakthroughIndicators).toContain('self_realization')
      expect(contextualFactors.therapeuticFactors!.progressMarkers).toContain('behavioral_insight')
      
      // Relationship dynamics should reflect professional boundaries
      expect(contextualFactors.relationshipSpecificFactors.professionalBoundaries).toBe(true)
      expect(contextualFactors.relationshipSpecificFactors.guidanceStyle).toBe('reflective')
  ...
  describe('Temporal Context Integration', () => {  
    it('should apply late-night vulnerability adjustments to mood analysis', async () => {
      ...
      // Context-enhanced analysis
      const contextualFactors = contextBuilder.identifyContextualFactors(lateNightConversation)
      ...
      expect(contextualFactors.temporalFactors!.emotionalVulnerability).toBe('heightened')
      expect(contextualFactors.temporalFactors!.contextualTriggers).toContain('emotional_fatigue')
      expect(contextualFactors.temporalFactors!.contextualTriggers).toContain('reduced_emotional_barriers')
      
      // Primary triggers should include temporal vulnerability
      expect(contextualFactors.primaryTriggers).toContain('temporal_vulnerability')
      
      // Late-night conversations should be weighted differently
      expect(contextualFactors.factorWeighting!.temporalWeight).toBeGreaterThan(0.1)
    })
    ...
    it('should integrate stress accumulation patterns into mood scoring', async () => {
      ...
      // Context-enhanced analysis
      const contextualFactors = contextBuilder.identifyContextualFactors(stressConversation)
      
      // Context should indicate stress accumulation
      expect(contextualFactors.stressFactors!).toBeDefined()
      expect(contextualFactors.stressFactors!.accumulationPattern).toBe('escalating')
      expect(contextualFactors.stressFactors!.durationImpact).toBe('significant')
      expect(contextualFactors.stressFactors!.fatigueIndicators).toContain('cognitive_overload')
    ...
    it('should contextualize major life transitions in mood analysis', async () => {
      ...
      // Context-enhanced analysis
      const contextualFactors = contextBuilder.identifyContextualFactors(lifeTransitionConversation)
      
      // Context should indicate major life transition
      expect(contextualFactors.lifeEventFactors!).toBeDefined()
      expect(contextualFactors.lifeEventFactors!.eventType).toBe('major_transition')
      ...
      expect(contextualFactors.lifeEventFactors!.adaptationChallenges).toContain('uncertainty_management')
      
      // Primary triggers should include life transition stress
      expect(contextualFactors.primaryTriggers).toContain('life_transition_stress')
    })
  })
  ...
  describe('Baseline-Aware Context Integration', () => {
    it('should integrate emotional baseline context for personalized mood assessment', async () => {
      ...
      const contextualFactors = contextBuilder.identifyContextualFactors(currentConversation)
      const deviationAnalysis = baselineManager.analyzeDeviation(userId, currentConversation)
    ...
    it('should apply relationship-specific baseline adjustments', async () => {
      ...
      const baseline = baselineManager.establishBaseline(userId, historicalConversations)
      
      // Current romantic conversation should be compared to romantic baseline, not overall
      const currentRomanticConversation = createRelationshipConversation(userId, 6.0, 'romantic')
      
      // Context-enhanced analysis
      // const contextualFactors = contextBuilder.identifyContextualFactors(currentRomanticConversation) // Placeholder for future use
      const deviationAnalysis = baselineManager.analyzeDeviation(userId, currentRomanticConversation)
      ...
      expect(baseline.relationshipPatterns.romantic.averageMood).toBeGreaterThan(8.0)
      expect(baseline.relationshipPatterns.professional).toBeDefined()
      expect(baseline.relationshipPatterns.professional.averageMood).toBeLessThan(6.5)
    })
  })
  ...
  describe('Comprehensive Context Integration', () => {
    it('should provide multi-dimensional context-aware mood analysis', async () => {
      ...
      const contextualFactors = contextBuilder.identifyContextualFactors(comprehensiveConversation)
      ...
      expect(contextualFactors.contextualSummary!.dominantContext).toBe('emotional_support')
      expect(contextualFactors.contextualSummary!.secondaryContexts).toContain('vulnerability_sharing')
      expect(contextualFactors.contextualSummary!.emotionalClimate).toBe('supportive')
      
      // Should provide comprehensive weighting
      expect(contextualFactors.factorWeighting!).toBeDefined()
      expect(contextualFactors.factorWeighting!.relationshipWeight).toBeGreaterThan(0.4)
      expect(contextualFactors.overallContextConfidence).toBeGreaterThan(0.8)
      
      // Should recommend mood adjustments based on context
      expect(contextualFactors.contextualSummary!.recommendedMoodAdjustments).toContain('baseline_elevation')
      expect(contextualFactors.contextualSummary!.supportiveElements).toContain('active_listening')
      
      // Risk factors should be contextually appropriate
      expect(contextualFactors.contextualSummary!.riskFactors).toContain('emotional_overwhelm')
    })
    ...
    it('should demonstrate context sensitivity in mood scoring accuracy', async () => {
      ...
      const testScenarios = [
        { conversation: createVulnerableConversation(), expectedAdjustment: 'support_amplification' },
        { conversation: createConflictConversation(), expectedAdjustment: 'conflict_penalty' },
        { conversation: createCelebrationConversation(), expectedAdjustment: 'achievement_boost' },
        { conversation: createTherapeuticConversation(), expectedAdjustment: 'therapeutic_progress' },
        { conversation: createLateNightConversation(), expectedAdjustment: 'temporal_vulnerability' },
      ]
      
      for (const scenario of testScenarios) {
        const contextualFactors = contextBuilder.identifyContextualFactors(scenario.conversation)
        const relationshipDynamics = relationshipAnalyzer.analyzeRelationshipDynamics(scenario.conversation)
        
        // Each scenario should produce contextually appropriate analysis
        expect(contextualFactors.contextualSignificance).toBeOneOf(['medium', 'high'])
        ...
        expect(['high', 'medium', 'low']).toContain(relationshipDynamics.intimacy)
        expect(['high', 'medium', 'low', 'negative']).toContain(relationshipDynamics.supportLevel)
        expect(['high', 'medium', 'low']).toContain(relationshipDynamics.emotionalSafety.overall)
      }
    })
  })
})
...
function createVulnerableConversation(): ConversationData & { moodAnalysis: MoodAnalysisResult } {
  return {
    ...
    messages: [
      {
        id: 'msg1',
        content: 'I\'ve been struggling with something really personal and I don\'t know who else to talk to about this',
      ...
      {
        id: 'msg3',
        content: 'I\'m so scared about how people will react if they find out. I feel like I\'m living a lie',
        authorId: 'vulnerable-user',
...
function createConflictConversation(): ConversationData & { moodAnalysis: MoodAnalysisResult } {
  return {
    id: 'conflict-conv-1',
    ...
    messages: [
      {
        id: 'msg1',
        content: 'I can\'t believe you\'re dismissing my feelings again. This always happens when I try to talk about something important',
        authorId: 'user1',
        timestamp: new Date('2024-01-15T18:45:00Z'),
      },
      {
        id: 'msg2',
        content: 'You\'re being way too sensitive about this. It\'s not that big of a deal',
        authorId: 'user2',
        timestamp: new Date('2024-01-15T18:47:00Z'),
      },
      {
        id: 'msg3',
        content: 'See? There you go again, invalidating how I feel. I\'m done trying to explain this to you',
        authorId: 'user1',
...
function createCelebrationConversation(): ConversationData & { moodAnalysis: MoodAnalysisResult } {
  return {
    ...
    messages: [
      {
        id: 'msg1',
        content: 'I got the promotion! I can\'t believe it actually happened after all the hard work',
        authorId: 'achiever',
        timestamp: new Date('2024-01-15T12:00:00Z'),
      },
      {
        id: 'msg2',
        content: 'That\'s incredible! I\'m so proud of you. You absolutely deserved this recognition',
        authorId: 'celebrator',
        timestamp: new Date('2024-01-15T12:02:00Z'),
      },
      {
        id: 'msg3',
        content: 'Thank you for believing in me when I didn\'t believe in myself. Your support means everything',
        authorId: 'achiever',
...
function createTherapeuticConversation(): ConversationData & { moodAnalysis: MoodAnalysisResult } {
  return {
    ...
    messages: [
      {
        id: 'msg1',
        content: 'I keep having the same argument with people and I don\'t understand why it keeps happening',
        authorId: 'client',
        timestamp: new Date('2024-01-15T16:00:00Z'),
      },
      {
        id: 'msg2',
        content: 'What do you notice about the pattern in these arguments? What themes come up for you?',
        authorId: 'therapist',
        timestamp: new Date('2024-01-15T16:02:00Z'),
      },
      {
        id: 'msg3',
        content: 'Now that you ask... I think I always feel like I\'m not being heard. Oh wow, that connects to my childhood',
        authorId: 'client',
      ...
      factors: [
        {
          ...
          evidence: ['pattern recognition', 'insight moment', 'connecting themes'],
        },
      ],
    },
  } as ConversationData & { moodAnalysis: MoodAnalysisResult }
...
function createLateNightConversation(): ConversationData & { moodAnalysis: MoodAnalysisResult } {
  return {
    ...
    messages: [
      {
        id: 'msg1',
        content: 'I can\'t sleep again. My mind just won\'t stop racing about everything that\'s wrong in my life',
        authorId: 'night-user',
        timestamp: new Date('2024-01-15T23:45:00Z'),
      },
      {
        id: 'msg2',
        content: 'It\'s so late, are you okay? Do you want to talk about what\'s keeping you up?',
        authorId: 'supporter',
        timestamp: new Date('2024-01-15T23:47:00Z'),
      },
      {
        id: 'msg3',
        content: 'Everything feels so much worse at night. I feel so alone and scared about the future',
        authorId: 'night-user',
...
function createProlongedStressConversation(): ConversationData & { moodAnalysis: MoodAnalysisResult } {
  return {
    id: 'stress-conv-1',
    ...
    messages: [
      {
        id: 'msg1',
        content: 'Work has been absolutely crushing me lately. I can\'t keep up with all the demands',
        authorId: 'stressed-user',
        timestamp: new Date('2024-01-15T10:00:00Z'),
      },
      {
        id: 'msg2',
        content: 'That sounds really overwhelming. How long has this been building up?',
        authorId: 'listener',
        timestamp: new Date('2024-01-15T10:05:00Z'),
      },
      {
        id: 'msg3',
        content: 'Months now. And it\'s affecting everything - my sleep, my relationships, my health. I feel like I\'m drowning',
        authorId: 'stressed-user',
...
function createLifeEventConversation(): ConversationData & { moodAnalysis: MoodAnalysisResult } {
  return {
    ...
    messages: [
      {
        id: 'msg1',
        content: 'Moving across the country for this new job is exciting but also terrifying. I\'m leaving everything I know',
        authorId: 'transitioning-user',
        timestamp: new Date('2024-01-15T15:30:00Z'),
      },
      {
        id: 'msg2',
        content: 'Big life changes are always scary, even the good ones. It\'s normal to feel mixed emotions about this',
        authorId: 'confidant',
        timestamp: new Date('2024-01-15T15:35:00Z'),
      },
      {
        id: 'msg3',
        content: 'What if I can\'t make new friends? What if I hate the new city? There are so many unknowns',
        authorId: 'transitioning-user',
...
function createComprehensiveContextConversation(): ConversationData & { moodAnalysis: MoodAnalysisResult } {
  return {
    ...
    participants: [
      {
        ...
        role: 'vulnerable_sharer',
        messageCount: 9,
        emotionalExpressions: ['vulnerable', 'hopeful', 'grateful', 'uncertain'],
      },
      {
        ...
        role: 'emotional_leader',
        messageCount: 8,
        emotionalExpressions: ['empathetic', 'wise', 'protective', 'encouraging'],
      },
    ],
    messages: [
      {
        id: 'msg1',
        content: 'I wanted to update you on how therapy has been going. It\'s been really intense but also eye-opening',
        authorId: 'multi-faceted-user',
        timestamp: new Date('2024-01-15T21:00:00Z'),
      },
      {
        id: 'msg2',
        content: 'I\'m so proud of you for doing that work. What kinds of things are you discovering?',
        authorId: 'comprehensive-supporter',
        timestamp: new Date('2024-01-15T21:03:00Z'),
      },
      {
        id: 'msg3',
        content: 'A lot about my patterns in relationships and how my past affects my present. It\'s scary but liberating',
        authorId: 'multi-faceted-user',
        timestamp: new Date('2024-01-15T21:07:00Z'),
      },
      {
        id: 'msg4',
        content: 'Growth is never easy, but you\'re doing something so brave. I\'m here to support you through this',
        authorId: 'comprehensive-supporter',
      ...
      factors: [
        {
          ...
          evidence: ['therapy progress', 'self-discovery', 'pattern recognition'],
        },
        {
          type: 'relationship_context',
          weight: 0.3,
          description: 'Strong support system',
          evidence: ['emotional support', 'encouragement', 'unconditional presence'],
        },
      ],
    },
  } as ConversationData & { moodAnalysis: MoodAnalysisResult }
...
function createBaselineConversation(
  ...
  return {
    ...
    messages: [
      {
        ...
        content: 'This is a sample conversation message for baseline establishment.',
        authorId: userId,
        timestamp,
      },
      {
    ...
    moodAnalysis: {
      score: moodScore,
      descriptors: moodScore > 7 ? ['positive', 'uplifted'] : moodScore < 5 ? ['concerned', 'unsettled'] : ['neutral', 'balanced'],
      confidence: 0.85,
      factors: [
        {
